...
def beliefPropogation(EsN0, H, sortedActions, clusters, realizationMap, M, lmax, repre, thre):
	...
	while l < lmax:
		...
		for cluster in clusters:
			...
			'''
			for action, _ in sortedActions[i]:
...
with open("test-03-26.txt", 'rb') as f:
    Q = pickle.load(f)

print(f"Q Table dimensions: {len(Q)} x {len(Q[0])}")
...
print(f"sortedActions dimensions: {len(sortedActions)} x {len(sortedActions[0])}")


repre = list(np.load('repre_0.72.npy'))
for i in range(len(repre)):
	repre[i] = round(repre[i], 3)
thre = list(np.load('thre_0.72.npy'))
...
for i in range(numberOfTrials):
	...
	print(f"Vector {i}, {errors[0]/lengthOfCode}")

print(bitErrors/(numberOfTrials*lengthOfCode), blockErrors/numberOfTrials)
'''