...
def qLearning(H, lmax, e, clusters, M, numberOfVectors, EsN0, seed):
	...
	for _ in range(numberOfVectors):
		...
		while l < lmax:
			...
			# update syndrome
			updateSyndrome(edges, action, S_hat, L, repre, thre)
	...
	with open("resultsEsN0=0.05;B=B2.txt", "wb") as fp:
		pickle.dump(Q, fp)


# put the code rate here
# varn = 1/(2*R*pow(10, 0.1*EbN0))
def awgn(data, EsN0):
	variance = 1 / (2 * EsN0)
	dataLength = len(data)
...
def generateNoiseVectors(bitsWidth, numberOfVectors, EsN0):
	...
	for i in range(numberOfVectors):
		newArr = [1 for i in range(bitsWidth)]
