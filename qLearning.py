...
def qLearning(H, lmax, e, clusters, M, numberOfVectors, EsN0, seed, name, rep, thr):


	n = len(H)
	maxQ = 0

	repre = list(np.load(rep))
	for i in range(len(repre)):
		repre[i] = round(repre[i], 3)
	thre = list(np.load(thr))

	realizationMap = RealizationMap().generateRealizationMap(repre, len(clusters[0]))

	print(realizationMap)
...
today = date.today()
name = today.strftime("test-%m-%d.txt")

script_dir = os.path.dirname(__file__)
rep = f"quantization_files\repre_{EsN0}.npy"
thr = f"quantization_files/thre_{EsN0}.npy"

qLearning(H, lmax, e, clusters, M, numberOfVectors, EsN0, 0, name, rep, thr)
...
print(calculate_mean(S))

print(calculate_variance(S))
'''